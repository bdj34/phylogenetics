...
#write.csv(df, paste0("~/rotation_fall2021/clade_estimates_", date, ".csv"))
...
ggplot(plot.df, aes(x = driver_and_pid_and_age)) + geom_point(aes(y = their_growthRate), color = "blue") + 
  ...
  geom_point(aes(y = our_growthRate_theirTree), color = "red", fill = "black", shape = 22, size = 2, alpha = .8) +
  ...
  theme(axis.text.x = element_text(face = "bold", angle = 70, vjust = 1, hjust=1, size = 10),
